...
void ResourceManagerClass::update()
{
	...
	double gasRate = 0;
	for(std::map<Base, std::map<Unit, GatherTaskPointer>>::iterator iTask = mResourceTasks.begin(); iTask != mResourceTasks.end(); ++iTask)
	{
		for(std::map<Unit, GatherTaskPointer>::iterator iGatherTask = iTask->second.begin(); iGatherTask != iTask->second.end(); ++iGatherTask)
		{
			mineralRate += iGatherTask->second->getMineralRate();
			gasRate += iGatherTask->second->getGasRate();
		}
	}
