...
void UnitClass::drawUnitPosition()
{
	...
	// Idle larvas just bloat the screen
	if (type == BWAPI::UnitTypes::Zerg_Larva && !isMorphing())
	    return;
...
// 	BWAPI::Broodwar->drawTextMap(pos.x() + type.dimensionRight(), pos.y()+40, "Last cmd: %s", mUnit->getLastCommand().getType().getName().c_str());
// 	
// 	if(isBurrowed())
// 		BWAPI::Broodwar->drawTextMap(pos.x() + type.dimensionRight(), pos.y()+50, "Burrowed");
