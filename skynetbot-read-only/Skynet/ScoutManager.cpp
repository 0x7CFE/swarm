...
//#endif
	...
	updateWorkerScouts();
//	if (Condition(ConditionTest::isResearched, BWAPI::UpgradeType::Ventral_Sacs).evauluate())
	    updateOverlordScouts();
...
// 	BWAPI::Broodwar->drawTextScreen(200, 0, "Max Worker Scouts : %u", maxForThisTime);
// 	BWAPI::Broodwar->drawTextScreen(200, 10, "Current Worker Scouts : %u", mWorkerScouts.size());
...
{
	for(std::set<OverlordScoutTaskPointer>::iterator it = mOverlordScouts.begin(); it != mOverlordScouts.end();)
	{
		if((*it)->hasEnded())
	...
	// For the first 4 minutes we may safely scout with all available Overlords, 
	// after that need to get some reserve depending on the supply requirements
	
	unsigned int curreentOverlords = UnitTracker::Instance().selectAllUnits(BWAPI::UnitTypes::Zerg_Overlord).size();
	unsigned int maxOverlords = BWAPI::Broodwar->getFrameCount() < 24*60*4 ? curreentOverlords : (curreentOverlords > 1 ? curreentOverlords - 1 : 0);
	//BWAPI::Broodwar->drawTextScreen(200, 35, "Max Overlord Scouts : %u", maxOverlords);
	//BWAPI::Broodwar->drawTextScreen(200, 45, "Current Overlord Scouts : %u", mOverlordScouts.size());
