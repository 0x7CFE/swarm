...
void ResourceManagerClass::update()
{
	...
	{
	    unsigned int availableLarva = UnitTracker::Instance().selectAllUnits(BWAPI::UnitTypes::Zerg_Larva).size();
	    
	    // Selecting all available homes
	    UnitGroup hatcheries = 
		UnitTracker::Instance().selectAllUnits(BWAPI::UnitTypes::Zerg_Hatchery) +
		UnitTracker::Instance().selectAllUnits(BWAPI::UnitTypes::Zerg_Lair) + 
		UnitTracker::Instance().selectAllUnits(BWAPI::UnitTypes::Zerg_Hive);
	    
	    // Filtering that being built
	    for (UnitGroup::iterator hatchery = hatcheries.begin(); hatchery != hatcheries.end(); )
	    {
		if (! hatchery->get()->isCompleted())
		    hatcheries.erase(hatchery++);
		else
		    ++hatchery;
	    }
	    
	    // Each home supplies 3 larva
	    unsigned int maxLarva = hatcheries.size() * 3;
	    
	    // Reserving at most 30% of larva for workers
	    neededWorkers = std::min(neededWorkers, (unsigned int)(maxLarva / 3) + 1);
	    BWAPI::Broodwar->drawTextScreen(150, 4, "Larva Max/Avl/RFW %d/%d/%d", maxLarva, availableLarva, neededWorkers);
	}
	else 
